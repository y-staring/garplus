...
class NonMolecularVisualization:
    ...
    def visualize_non_molecule(self, graph, pos, path, iterations=100, node_size=100, largest_component=False):
        ...
        # ====== 2. 节点颜色：按类型（color_val）染色 ======
        # color_val 是你在 to_networkx 里写入的节点类型/度分箱
        node_color_vals = []
        for n in graph.nodes():
            node_color_vals.append(graph.nodes[n].get("color_val", 0))
    
        # 使用一个离散的 colormap（tab20 之类）
        import numpy as np
        node_color_vals = np.array(node_color_vals)
        if len(node_color_vals) > 0:
            vmin = node_color_vals.min()
            vmax = node_color_vals.max()
        else:
            vmin, vmax = 0, 1
        ...
        #     et = int(data.get("color", 0))  # 0,1,2
        #     if et == 1:
        #         edge_colors.append("gray")       # 负边
        #         edge_widths.append(2.0)
        #         edge_styles.append("dashed")
        #     elif et == 2:
        #         edge_colors.append("black")        # 正边
        #         edge_widths.append(2.0)
        #         edge_styles.append("solid")
        #     else:
        #         edge_colors.append("gray")       # 无边（通常不会画）

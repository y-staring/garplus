...
class SpectreSamplingMetrics(nn.Module):
    def __init__(self, datamodule, compute_emd, metrics_list):
        ...
        # self.G_large = load_epinions_graph()
    ...
    def forward(self, generated_graphs: list, name, current_epoch, val_counter, local_rank, test=False):
        ...
        # support_results = compute_support_per_graph(self.G_large, networkx_graphs)
        # print("\n==== Support per generated graph ====")
        # for item in support_results:
        #     print(f"Graph {item['id']:3d}  |  support = {item['support']}")
        # print("=====================================\n")
        # upports = [d["support"] for d in support_results]

        # print("\n==== Support summary ====")
        # print(f"num_graphs = {len(supports)}")
        # print(f"min support = {min(supports)}")
        # print(f"max support = {max(supports)}")
        # print(f"avg support = {sum(supports) / len(supports):.4f}")
        # print("================================\n")

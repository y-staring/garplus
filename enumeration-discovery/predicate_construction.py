...
def build_global_predicate_table(
    ...
    for pattern_id, pattern_graph in patterns:
        ...
        for pred in repository.predicates:
            count = evaluate_predicate_count_on_pattern(pred, pattern_graph, global_graph)
            if mode == "binary":
...
def run_global_predicate_repository_pipeline(
    ...
    binary_table = build_global_predicate_table(patterns, repository, global_graph, mode="binary")
    count_table = build_global_predicate_table(patterns, repository, global_graph, mode="count")
    binned_table = build_global_predicate_table(patterns, repository, global_graph, mode="binned")
    ...
    print("[Info] Learning family-wise Predicate-BNs from binned-count predicate table...")
    family_results = learn_family_predicate_bns(
        full_table=binned_table,
        repository=repository,
        output_dir=output_dir,
        min_support=MIN_PREDICATE_SUPPORT,

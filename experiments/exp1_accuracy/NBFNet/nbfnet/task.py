...
@R.register("tasks.RelationClassification")
class RelationClassification(tasks.Task, core.Configurable):
    def __init__(self, model, num_classes=3):
        super(RelationClassification, self).__init__()
        self.model = model
        self.num_classes = num_classes
        self.register_buffer("class_id", torch.arange(num_classes, dtype=torch.long))
    ...
    def preprocess(self, train_set, valid_set, test_set):
        ...
        train_mask = train_set.indices
        train_graph = dataset.graph.edge_mask(train_mask)
        # 对于关系分类任务，我们通常在验证和测试时也使用训练集构建的图进行消息传递
        # 从而避免 label 泄露
        self.register_buffer("train_graph", train_graph)
        self.register_buffer("valid_graph", train_graph)
        self.register_buffer("test_graph", train_graph)
    ...
    def predict_and_target(self, batch, all_loss=None, metric=None):
        ...
        # 把所有候选关系打平成一个更大的 batch，一次性前向，避免对同一张图重复跑 num_classes 次。
        h = h_index.unsqueeze(-1).expand(-1, self.num_classes).reshape(-1, 1)
        t = t_index.unsqueeze(-1).expand(-1, self.num_classes).reshape(-1, 1)
        r = self.class_id.unsqueeze(0).expand(batch_size, -1).reshape(-1, 1)

        pred = self.model(graph, h, t, r, all_loss=all_loss, metric=metric)
        pred = pred.view(batch_size, self.num_classes)
        return pred, r_index
    ...
    def evaluate(self, pred, target):
        ...
        metric["f1_macro"] = f1_score(labels, preds, average="macro")
        
        try:
            metric["auc"] = roc_auc_score(labels, probs, multi_class="ovr")
        except:
            metric["auc"] = 0.0
            
        pre_per_class = precision_score(labels, preds, average=None, labels=list(range(self.num_classes)), zero_division=0)
        rec_per_class = recall_score(labels, preds, average=None, labels=list(range(self.num_classes)), zero_division=0)
        
        metric["pre_macro"] = pre_per_class.mean()
        metric["rec_macro"] = rec_per_class.mean()
        if self.num_classes > 2:
            metric["rec_label2"] = rec_per_class[2]
            metric["pre_label2"] = pre_per_class[2]
